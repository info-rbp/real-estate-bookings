...
function normalizeMembershipPlanCode(planCode: string) {
...
function deriveStripeSubscriptionId(eventType: string, object: Record<string, unknown>) {
...
function extractStripeCustomerId(object: Record<string, unknown>) {
  const customer = object.customer;
  if (typeof customer === "string") {
    return customer;
  }

  return String(toRecord(customer).id || "");
}

function extractStripePriceIds(object: Record<string, unknown>) {
  ...
  const items = toRecord(object.items);
  const itemData = Array.isArray(items.data) ? items.data : [];
  itemData.forEach(addItem);

  const lines = toRecord(object.lines);
  const lineData = Array.isArray(lines.data) ? lines.data : [];
  lineData.forEach(addItem);

  return [...priceIds];
}

async function derivePlanCodeFromStripePrice(priceIds: string[]) {
  if (!priceIds.length) return "";

  const admin = createAdminContext();
  for (const priceId of priceIds) {
    const plan = await admin.findOne<Record<string, unknown>>(
      collectionIds.membershipPlans,
      [Query.equal("stripe_price_id", [priceId])],
    );
    if (plan?.plan_code) {
      return normalizeMembershipPlanCode(String(plan.plan_code));
    }
  }
...
async function handleStripeWebhook(context: FunctionContext) {
  ...
  let planCode = normalizeMembershipPlanCode(String(metadata.plan_code || ""));
  const stripeSubscriptionId = deriveStripeSubscriptionId(event.type, object);
  const stripeCustomerId = extractStripeCustomerId(object);
  ...
  if (!tenantId && stripeCustomerId) {
    const customerRecord = await admin.findOne<Record<string, unknown>>(
      collectionIds.stripeCustomers,
      [Query.equal("stripe_customer_id", [stripeCustomerId])],
    );
    tenantId = String(customerRecord?.tenant_id || "");
  }
  ...
  if (!planCode && currentSubscription?.plan_code) {
    planCode = normalizeMembershipPlanCode(String(currentSubscription.plan_code));
  }

  if (!planCode) {
    planCode = await derivePlanCodeFromStripePrice(extractStripePriceIds(object));
  }
  ...
  if (tenantId) {
    const existingPlanCode = normalizeMembershipPlanCode(String(currentSubscription?.plan_code || ""));
    const subscriptionPlanCode = planCode || existingPlanCode || (status === "active" ? "premium" : "free");
    if (currentSubscription && "$id" in currentSubscription) {
      ...
      await admin.createDocument(collectionIds.subscriptions, {
        tenant_id: tenantId,
        plan_code: subscriptionPlanCode,
        status,
    ...
    if (status === "active") {
      await grantTenantEntitlements(tenantId, subscriptionPlanCode);
  ...
  await persistAuditEvent({
    eventName: "stripe_webhook_processed",
    tenantId: tenantId || undefined,
    payload: { eventType: event.type, eventId: event.id, planCode, status, stripeSubscriptionId },
  });
